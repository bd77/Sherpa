...
Adapted for Slovenia on 16 January 2017

ONE SCRIPT TO RUN THEM ALL
- EMEP and CHIMERE based sherpa
- PM2.5
- no source approtionament per Area
- all snaps or per snap
- all precursors or per precursor

Each run calls module 6 (modified with full output)
- target cells are points provided by Jana
concentration changes due to emission reductions in
- 5 percursors
- 10 sectors
- emissions in the whole domain
...
# read file with receptor points
# Country;City;lat;lon
wd = "D:/SHERPA/Slovenia/"
fcities = open(wd + "Slovenia_cities.txt", 'r')
fcities.readline()     # read header
city_dict = {}
while True:
    line = fcities.readline().rstrip()
    if len(line) == 0:
        break
    [country_code, cityname, lat, lon] = line.split(';')
...
model_file = wd + 'Sherpa/config_file_20180911.txt'
...
results_path = '../Sherpa_results/%s_%s_%s/' % (date_tag, snap_tag, precursor_tag)
...
for model_name in model_dict.keys():
    ...
    if os.path.exists(results_file_name):
        ...
        fallres.write('commit xxxxxxxxxxxxxxxxxxxxxx\n')
...
#                source_area_dict[source_area_list[i].replace('.nc', '')] = reduc_area_all_path + source_area_list[i]
        ...
        # It's very simple, there's only one source area, the full domain
        source_area_dict = {}
        source_area_dict["full_domain"] = reduc_area_path + "_full_domain.nc"
            ...
            for user_reduction_file in user_reduction_list:
                ...
                if pollutant_tag != 'NO2' or (pollutant_tag == 'NO2' and precursor == 'NOx'):
                    ...
                    if done_or_not_done:
                        ...
                        # call module 6
                        # print(path_emission_cdf, fua_area_cdf, target_cell_lat, target_cell_lon, path_reduction_txt, path_base_conc_cdf, path_model_cdf, path_cell_surface_cdf, path_result_cdf)
